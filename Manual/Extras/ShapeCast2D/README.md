...
O node `ShapeCast2D` é muito parecido com o `RayCast2D` visto [anteriormente](https://github.com/felipebottega/Games/tree/gh-pages/Manual/Physics/Ray-casting), com a diferança que o `ShapeCast2D` emite um shape em vez de uma linha para detectar colisões. 

## Criando um ShapeCast2D

O processo de criação também é parecido. Adicione o node `ShapeCast2D` à sua cena e, no *Inspector*, selecione o shape que você quer. A maior parte das propriedades são as mesmas do `RayCast2D`, e as diferentes não são muito complicadas de entender.
...
Depois de selecionar um shape, você pode clicar no nome do shape selecionada novamente para abrir mais opções de customização do shape.
...
Diferentemente do `RayCast2D`, é possível obter múltiplas detecções de colisão ao mesmo tempo. Para isso acontecer, é necessário que os corpos detectados estejam dentro da mesmo shape no instante da colisão, como indicado na figura abaixo. A lógica de detectar colisões é igual ao `RayCast2D`, isto é, assim que uma detecção ocorre, o `ShapeCast2D` não tenta mais buscar colisões naquele frame físico. Por isso que múltiplas colisões só são possíveis se os corpos estiverem dentro da mesmo shape usada para detecção no mesmo instante.
