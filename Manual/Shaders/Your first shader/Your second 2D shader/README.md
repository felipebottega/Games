...
<p align="center">
  <img width="280" src="https://github.com/user-attachments/assets/f57fa25f-8168-4712-ade2-58db3077fe3d" />
</p>
...
-  Funções que aplicam os efeitos. O valor do int `effect` que determina qual função será usada.
-  Função de processamento `fragment()`:
    - Declaração de variáveis comuns a vários efeitos.
    - Inicialização de um `switch`, em que cada caso é a escolha de um efeito (determinado pela variável `effect`). Assim que cai um algum dos casos, a função correspondente é executada.
...
Vamos explicar apenas o primeiro efeito dos que foram utilizados neste projeto. A ideia é entrar em detalhes neste efeito e esperar que isso sirva de base para poder entender os outros por conta própria depois.
...
Supondo que o jogador não mexa nos parâmetros `frequency`, `speed` e `amplitude`, eles podem ser considerados constantes na fórmula. Em outras palavras, cada escolha destes parâmetros determina uma escolha de efeito wave. Deste modo, só o que varia é o tempo e a coordenada $x$. Note que, em um instante `TIME`, a linha horizontal associada a um $y$ fixo só varia com $x$. Isso quer dizer que existe variação horizontal dependente de $x$. Por outro lado, se for $x$ fixo e $y$ variando em um mesmo instante `TIME`, temos que todos os valores `uv.y` recebem incrementos iguais. Isso quer dizer que todos os pixels de uma coluna sobem ou descem iguais. 
...
Uma vez que o ponto `uv` foi atualizado, isso deve ser repassado para o pixel, em um `vec4` para atualizar o `COLOR`. A função `texture()` deve receber a textura original (variável `tex`) e o novo ponto `uv`. Essa função retorna o RGBA do `uv` da textura. Como a coordenada $y$ foi alterada, isso significa que o retorno `texture(tex, uv)` da função vai retonar um ponto mais acima ou mais abaixo do original. Esse é o comportamento que queremos, apenas subir e descer pixels.

<p align="center">
  <a href="https://github.com/felipebottega/Games/tree/gh-pages/Manual/Shaders/Your%20first%20shader/Your%20first%202D%20shader">⬅ Anterior</a>  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
  <a href="https://github.com/felipebottega/Games/tree/gh-pages/Manual/Shaders/Your%20first%20shader/Your%20third%202D%20shader">Próximo ➡</a>
</p>
