...
<p align="center">
  <img width="400" src="https://github.com/user-attachments/assets/50e4a95a-1366-4ec0-90cf-6ef62c65161d" />
</p>
...
Ao habilitar a evitação, o servidor de navegação cria um círculo em torno do agente. Este círculo não interfere no caminho de navegação. Todos os agentes e seus círculos ficam em um certo espaço separado dos outros espaços do jogo. Quando dois círculos deste espaço se tocam, o agente reconhece que esbarrou em outro agente com evitação habilitada, e assim eles evitam se sobrepor. 
...
- **Neighbor Distance:** É o raio de busca do agente, em pixels. Para ele saber se há outros agentes com evitação habilitada por perto, ele faz uma busca a cada frame físico. Reduzir este raio melhora a performance, mas pode fazer com que o agente só detecte outros agentes muito em cima da hora.
- **Max Neighbors:** Ao fazer a busca por outros agentes com evitação, você pode limitar o número de agentes encontrados para decidir que não é necessário mais buscar nada. Por exemplo, se este valor for igual a $1$, o agente para de buscar outros agentes com evitação assim que encontra o primeiro (isso é por frame, no frame seguinte ele fará a busca novamente).
...
No uso cotidiano do português, a palavra "velocidade" costuma ser empregada para indicar apenas a rapidez de um movimento, ou seja, um valor escalar. Caso seja o vetor de velocidade, é comum usar o termo "velocidade vetorial". No inglês, e na Godot, o termo "velocity" é usado para o vetor de velocidade. Para a "velocidade escalar" eles usam a palavra "speed". Apesar de ser um detalhe, é bom deixar isso explícito uma vez que o uso da palavra na língua portuguesa difere do uso na Godot. Eu já usei "velocidade" muitas vezes para me referir a rapidez, não ao vetor. Provavelmente ainda farei isso mais vezes, mas o contexto deixará claro o que quero dizer.
...
<p align="center">
  <img width="380" src="https://github.com/user-attachments/assets/ffd13854-3f36-4cbd-bcd6-d1d5107e675c" />
</p>

Colocamos abaixo o script usual à esquerda, onde o agente se move normalmente pelo caminho. Na direita, temos o caminho com a evitação implementada. No script usual, a nova direção calculada é passada diretamente para o vetor `velocity` do `CharacterBody2D`. No outro script, essa velocidade é passada para o `$NavigationAgent2D.velocity`. Isso ativa o sinal de fazer o servidor calcular um novo vetor com as evitações. Aí sim, dentro da função do sinal, o vetor seguro é passado para o `CharacterBody2D`.
...
Você pode testar o efeito dos parâmetros [neste simulador](https://felipebottega.github.io/Games/Manual/Navigation/Using%20NavigationAgents/html/) que eu implementei. 

<p align="center">
  <a href="https://github.com/felipebottega/Games/tree/gh-pages/Manual/Navigation/Using%20NavigationPathQueryObjects">⬅ Anterior</a>  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
  <a href="https://github.com/felipebottega/Games/tree/gh-pages/Manual/Navigation/Using%20NavigationObstacles">Próximo ➡</a>
</p>
