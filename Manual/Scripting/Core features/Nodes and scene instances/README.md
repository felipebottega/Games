...
Após o passo 1, a variável `scene` é um recurso de cena compactado (classe `PackedScene` que representa um recurso), não um node. Para criar o node propriamente dito, você precisa do método `PackedScene.instantiate()`. Ele retorna uma árvore de nodes que você pode usar como filho do node que fez a chamada pelo script. O comando `var instance = scene.instantiate()` cria esta árvore de nodes, e o comando `add_child(instance)` adiciona esta árvore como filho do node que fez a chamada. 
