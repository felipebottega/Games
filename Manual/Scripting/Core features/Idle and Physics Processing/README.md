...
Os jogos são executados em um loop. A cada frame, a engine precisa atualizar o estado dos elementos do jogo antes de desenhá-los na tela. A Godot fornece dois métodos virtuais na classe `Node` para isso: `Node._process()` e `Node._physics_process()`. Se você definir um ou ambos em um script, a engine os chamará automaticamente.
