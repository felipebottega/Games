...
O range dos valores mostrados no *Inspector* devem ser interpretados como a posição em relação ao node pai, sendo $0$ na origem e $1$ na posição oposta do outro lado. Esses valores apenas fazem efeito caso o node pai seja um `CanvasLayer` seja um node de `Control`. Por exemplo, se for um botão como filho de um `CanvasLayer` ou um `ColorRect`, vai funcionar. Mas se o botão for filho de um `Node2D`, apenas os valores de margem (*anchor offsets*) fazem efeito. Neste caso, a âncora é fixada no canto superior esquerdo do node pai.

## Anchor Presets

Existem alguns atalhos para obter valores de margem mais usuais, sem precisar fazer manualmente. Você pode clicar na opção que aparece assim do espaço 2D ou pode ir em *Inspector → Control → Layout → Anchors Presets* e selecionar uma das outras opções além da *Custom*.

<p align="center">
  <img width="750" src="https://github.com/user-attachments/assets/e9e62027-ef35-41dc-8463-a885946852d6" />
  <img width="250" src="https://github.com/user-attachments/assets/425b5609-f7ec-4b26-823f-655866e1bc5d" />
</p>
