...
Este, de longe, me parece ser o tipo de track mais interessante de todos. O track do tipo *call method* permite chamar uma função em um momento especificado dentro de uma animação. Por exemplo, você pode chamar `queue_free` para excluir um node no final de uma animação de morte. Clique em *+ Add Track* e selecione *Call Method Track*. Uma janela com a árvore de cena vai se abrir. Escolhe o node que quer animar. Nós começamos criando uma animação de posição para a espada e depois adicionamos a track de *call method*, como mostrado abaixo.
