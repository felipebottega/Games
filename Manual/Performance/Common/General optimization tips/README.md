...
3. Caso a busca binária acima não funcione por não ser possível comentar trechos do código, você pode colocar medições de tempo dentro de trechos do código para saber quanto tempo leva (coloque prints para mostrar os tempos no console). Você pode usar os comandos `var start = Time.get_ticks_usec()` e `var end = Time.get_ticks_usec()` em torno do trecho de código para este monitoramento. Depois é só fazer `end - start` para saber quanto tempo levou.
