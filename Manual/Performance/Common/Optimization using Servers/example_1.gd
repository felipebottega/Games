...
func _ready():
	...
	# Define como pai o CanvasItem interno pertencente a este Node2D. get_canvas_item() retorna o 
	# RID do CanvasItem usado pelo RenderingServer para o Node2D onde este script está anexado. A 
	# partir deste RID, a chamada canvas_item_set_parent() faz com que o RID deste CanvasItem seja o 
	# pai do item associado ao RID canvas_item_rid. Em resumo, get_canvas_item() passa a ser o pai 
	# de canvas_item_rid.
	...
	RenderingServer.canvas_item_add_texture_rect(canvas_item_rid, Rect2(-texture.get_size() / 2, texture.get_size()), texture)
	
	# Cria uma transformação (rotação, translação e escala).
	...
	RenderingServer.canvas_item_set_transform(canvas_item_rid, xform)
