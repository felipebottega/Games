...
A função nativa `_exit_tree()` é chamada automaticamente pela Godot quando um node está prestes a sair da árvore da cena (remoção, troca de cena, fechar o jogo, etc.). Ela é usada para limpeza final (parar threads, liberar recursos, etc.). 
