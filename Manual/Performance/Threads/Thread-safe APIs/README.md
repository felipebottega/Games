...
> PS: Tenha isso como regra geral: Thread serve pra pensar, não pra mexer no jogo.
  ...
  2. **Criar nodes e cenas fora da SceneTree:** Você pode fazer `load()`, `instantiate()`, entre outros, desde que não dê `add_child()` e não encoste na árvore ativa. Quando a thread terminar o trabalho, ela sinaliza a main thread. Aí a main thread faz `add_child()`. Isso é possível.
  ...
  4. **Usar chamadas de API para servidores:** Você pode usar servidores em thread se a thread não cria node, não chama `add_child()`, não acessa UI, só cria/configura coisas internas. Você não deve chamar se o método retorna node, o método depende de SceneTree, o método "parece alto nível".
