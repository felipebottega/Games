...
> PS: Tenha isso como regra geral: Thread serve pra pensar, não pra mexer no jogo.
  ...
  4. **Usar chamadas de API para servidores:** Você pode usar servidores em thread se não cria node, não chama `add_child()`, não acessa UI, só cria/configura coisas internas. Você não deve chamar se o método retorna node, o método depende de SceneTree, o método "parece alto nível". 
