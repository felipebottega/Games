...
## O que não pode fazer com threads
  ...
  1. **Não mexer na árvore de cena:** Por exemplo, comandos como `add_child()`, `remove_child()`, `queue_free()`, `get_node()` não podem ser chamados na thread.
...
> PS: Tenha isso como regra geral: Thread serve pra pensar, não pra mexer no jogo.
  ...
  2. **Criar nodes e cenas fora da árvore de cena:** Você pode fazer `load()`, `instantiate()`, entre outros, desde que não dê `add_child()` e não encoste na árvore ativa. Quando a thread terminar o trabalho, ela sinaliza a main thread. Aí a main thread faz `add_child()`. Isso é possível.
