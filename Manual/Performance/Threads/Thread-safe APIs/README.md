...
## O que não pode fazer com threads
  ...
  1. **Não mexer na SceneTree:** Por exemplo, comandos como `add_child()`, `remove_child()`, `queue_free()`, `get_node()` não podem ser chamados na thread.
  2. **Não mexer em UI:** Coisas como `Label.text = ""` ou `Button.disabled = true` não podem ser feitas na thread.
  3. **Não carregar o mesmo recurso em vários threads:** Por exemplo, ter duas (ou mais) threads carregando o mesmo *.tscn* é pedir para dar problema.
...
> PS: Tenha isso como regra geral: Thread serve pra pensar, não pra mexer no jogo.
  ...
  2. **Criar nós e cenas fora da SceneTree:** Você pode fazer `load()`, `instantiate()`, entre outros, desde que não dê `add_child()` e não encoste na árvore ativa. Quando a thread terminar o trabalho, ela sinaliza a main thread. Aí main thread faz `add_child()`. Isso é possível.
  ...
  4. **Usar chamadas de API para servidores:** Você pode usar servidores em thread se não cria node, não chama `add_child()`, não acessa UI, só cria/configura coisas internas. Você não deve chamar se o método retorna node, o método depende de SceneTree, o método "parece alto nível". Os servidores de renderização e física por default não são thread-safe. Para liberar isso, você deve ir em *Project → Project Settings → Rendering → Driver → Thread Model → Separate* e *Project → Project Settings → 2D (3D) → Run on Separate Thread*.
