...
Toda função de input pode ser chamada múltiplas vezes dentro da mesmo iteração (ou frame) do jogo. Ela não necessariamente é chamada apenas uma vez por iteração como a `_process`, por exemplo. Se mais de um evento de input ocorrer ao mesmo tempo, a função de tratar input será chamada uma vez para cada evento individual. Por exemplo, se você clicar e segurar o clique para arrastar um ícone, há o movimento do mouse e o botão do mouse sendo pressionado. Cada um é um evento de input distinto ocorrendo ao mesmo tempo. Isso significa que, a cada iteração do jogo, a função de tratar input será chamada 2 vezes (pelo menos). 
