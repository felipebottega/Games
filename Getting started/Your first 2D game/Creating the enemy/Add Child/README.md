...
No código abaixo, a variável `mob_scene` é como um molde da cena original. A variável `mob` é uma instância real da cena, ou seja, é um node que pode ser manipulado, movido, animado, receber sinais, etc. Porém, ele só passa a existir de fato na árvore de cena (SceneTree) do seu jogo depois de `add_child`. Se você executar o projeto agora, verá o ícone indicando a posição do `PathFollow2D` alterando de posição assim como antes, mas a cada posição que ele passa vai deixando um mob. Note que usamos o `global_position` para pegar a posição real do `PathFollow2D`, caso contrário ele consideraria que a trajetória começa na origem.
